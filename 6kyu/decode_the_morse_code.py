...
def decode_morse(morse_code):
    ...
    x = morse_code.replace("   ", " i ")
    print(x)
    list = x.split()  # moved untranslated words to a list
    ...
    for i in range(0, len(list)):
        if list[i] == 'i':
            ...
            translated = translated + MORSE_CODE[list[i]]
    translated = translated.strip() # remove whitespaces in front and back of string
    return(translated)
